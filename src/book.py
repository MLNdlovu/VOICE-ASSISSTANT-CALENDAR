...
# Volunteer-related functionality removed to focus on student bookings
...
def book_as_student(service,username,start_time,summary):
    """
    Books a time slot for a calendar event.

    Parameters:
    - service: An authenticated Google Calendar service object.
    - username: The username/email of the user booking the slot.
    ...
    This function books a time slot for a calendar event if the slot is available.
    It checks if the slot is free by verifying the number of attendees for the given start time.
    If the slot is available, the event is added to the Google Calendar.
    The event includes the user and any volunteers/attendees as attendees.

    Note:
    - The function relies on helper functions: get_attendee, search_time, and get_event_id.
    - If the slot is available, the function patches the existing event with attendees.
    ...
    try:
        ...
        if attendees and len(attendees) == 1:
            if len(search_time(start_time)) < 2:
                ...
                if not event_id:
                    print("Error: matching event not found in calendar.")
                    return
